module bridge_core::message_v2 {
    ...
    const EAUTHORIZATION_FAILED: u64 = 7;
    ...
    fun init_module(account: &signer) {
        move_to(
            ...
            MessageInfo {
                ...
                financer: signer::address_of(account)
            }
        );
    ...
    public entry fun set_financer(admin: &signer, new_financer: address) acquires MessageInfo {
        // Only admin can set the platform fee
        role_check(admin);

        let financer = &mut borrow_global_mut<MessageInfo>(@bridge_core).financer;
    ...
    public entry fun witdraw_platform_fee(
        ...
        let current_financer = borrow_global<MessageInfo>(@bridge_core).financer;
        assert!(current_financer == signer::address_of(financer), EAUTHORIZATION_FAILED);
        ...
        assert!(
            endless_coin::balance(signer::address_of(&message_signer)) >= amount,
            EINSUFFICIENT
        );
    ...
    #[view]
    /// withdraw the platform fee
    public fun get_platform_fee_balance(): u128 acquires MessageResource {
        let message_resource = borrow_global<MessageResource>(@bridge_core);
        let message_signer =
            account::create_signer_with_capability(&message_resource.signer_cap);
        return endless_coin::balance(signer::address_of(&message_signer))
    }
